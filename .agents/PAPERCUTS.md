...
- [ ] `2026-08-18T23:30:00Z` — `grok` — `src/types/schemas/reports.ts` sits on the oxlint 400-line ceiling (skipping blanks/comments), so adding a few Zod fields then running Prettier can fail `eslint/max-lines` even when the semantic change is small. Split more section schemas out of that file, or raise/disable the limit for this generated-looking contract.
- [ ] `2026-08-17T20:24:00Z` — `codex` — The full Vitest suite can time out otherwise-fast setup tests at the global 5-second limit under concurrent local load (`src/server/lib/dataforseo/client.test.ts` non-hosted billing and `src/server/mcp/oauth-provider.test.ts` provider construction both did so while 1,395 tests passed). Give integration-heavy setup tests an evidence-based per-test timeout or reduce suite worker contention so a busy development machine does not create false failures.
- [ ] `2026-08-17T20:17:00Z` — `codex` — `pnpm db:generate:d1` needs a TTY when Drizzle asks whether similarly shaped tables are renames, but in a non-interactive shell it prints `Interactive prompts require a TTY` and still exits with status 0. The generation script should fail nonzero or document/provide a deterministic non-interactive choice so CI and agents cannot mistake an ungenerated migration for success.
...
- [ ] `2026-08-18T11:56:00Z` — `claude` — Regenerating `src/routeTree.gen.ts` by calling `@tanstack/router-generator` directly (the workaround for the missing generate script) silently omits the trailing `declare module "@tanstack/react-start"` Register block that the Start plugin appends, which drops typed router inference and surfaces as unrelated `implicitly has an 'any' type` errors on `navigate({ search: (prev) => ... })` call sites. A `routes:generate` script driving the configured plugin would avoid the hand-repair.
...
- [x] `2026-08-16T21:58:25Z` — `codex` — A self-hosted image with precompiled `dist/` and a matching build fingerprint still failed on a fresh volume because Vite preview expects `.wrangler/deploy/config.json`, which only the skipped build creates. Resolved 2026-08-18 by making missing Wrangler deployment metadata a rebuild condition.
