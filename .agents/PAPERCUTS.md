...
- [ ] `2026-08-18T00:12:00Z` — `codex` — Orca `worker-start --worktree new-child` preserves child lineage in the UI but checks out the repository default branch instead of the active feature commit, so stacked workers silently start without their required foundation. Document or wrap parallel worker creation with an explicit Git-base argument/check before dispatch.
- [ ] `2026-08-16T21:24:06Z` — `codex` — Adding a TanStack file route makes `pnpm tsc --noEmit` fail against the stale generated route tree, but the repository has no route-generation script and does not install the `tsr` CLI. Running a full `vite build` regenerates `src/routeTree.gen.ts`, but is a slow workaround; expose the generator as a focused package script or make type-check generate routes first.
- [ ] `2026-08-16T21:12:54Z` — `codex` — Root scripts such as `pnpm db:generate` shell back into `npm run`, which causes npm to warn about every pnpm-only config environment key before each child command. Use `pnpm run` consistently inside package scripts so the declared pnpm toolchain does not hand its config to npm.
