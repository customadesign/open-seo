 ...
 *
 * Returns the AI visibility run to execute out of band, if one became due — the
 * caller runs it with `waitUntil` so a first visit never waits on live provider
 * calls.
 */
async function refreshMetricSnapshots(input: {
  ...
  languageCode: string;
  billingCustomer: BillingCustomerContext;
}): Promise<{ aiRunPlan: AiVisibilityRunPlan | null }> {
  ...
  if (ai.status === "rejected") {
    ...
    return { aiRunPlan: null };
  }
  return { aiRunPlan: ai.value.queued ? ai.value.plan : null };
}
