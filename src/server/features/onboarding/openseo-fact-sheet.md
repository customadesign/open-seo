...
OpenSEO is an open-source SEO platform for keyword research, domain research, backlinks, rank tracking, local SEO, site audits, monthly client reporting, Google Search Console, and AI-agent SEO workflows.
...
- Stored GoHighLevel Listings connections, DataForSEO-powered Maps geo-grid tracking, and evidence-based citation consistency audits. OpenSEO does not write to directories.
...
- Local SEO: store a canonical business profile, track Google Maps rankings across a geographic grid, review rank history and heatmaps, audit citation evidence, and open the connected GoHighLevel Listings account.
- Reports: publish immutable monthly snapshots from available rankings, Search Console, Analytics, Google Ads, audit, and backlink data. Staff manage report settings and commentary; client accounts receive read-only published-report access for assigned projects.
